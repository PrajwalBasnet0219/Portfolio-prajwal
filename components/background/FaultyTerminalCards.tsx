...
// CARD SUITS: 0=spade ♠ 1=heart ♥ 2=diamond ♦ 3=club ♣  — 5x5 bitmap
bool suitPixel(int suit, int ix, int iy){
  // spade
  if(suit==0){
    if(iy==0 && ix>=1 && ix<=3) return true;
    if(iy==1) return true;
    if(iy==2) return true;
    if(iy==3 && ix>=1 && ix<=3) return true;
    if(iy==4 && ix==2) return true;
    return false;
  }
  // heart
  if(suit==1){
  ...
  // diamond
  if(suit==2){
    if(iy==0 && ix==2) return true;
    if(iy==1 && ix>=1 && ix<=3) return true;
    if(iy==2) return true;
    if(iy==3 && ix>=1 && ix<=3) return true;
    if(iy==4 && ix==2) return true;
    return false;
  ...
  // club
  // club 5x5: three lobes + stem
  if(suit==3){
    if(iy==0 && ix>=1 && ix<=3) return true;
    if(iy==1 && (ix==0 || ix==2 || ix==4)) return true; // actually 1 1 0 1 1
    if(iy==1 && ix==1) return true;
    if(iy==1 && ix==3) return true;
    if(iy==2 && ix>=1 && ix<=3) return true;
    if(iy==3 && (ix==0 || ix==4)) return true;
    if(iy==3 && ix==2) return false;
    if(iy==3 && ix==1) return true;
    if(iy==3 && ix==3) return true;
    if(iy==4 && ix>=1 && ix<=3) return true;
    // fix club shape simplified
    if(iy==1){
      if(ix==0 || ix==1 || ix==3 || ix==4) return true;
...
float digit(vec2 p){
    ...
    int ix = int(floor(px5));
    int iy = int(floor(py5));

    // pick suit per cell
    float h = hash21(s);
    int suit = int(floor(h * 4.0));

    bool on = suitPixel(suit, ix, iy);

    float isOn = float(on) * step(0.12, intensity - 0.18);
...
interface FaultyTerminalCardsProps {
  scale?: number;
  gridMul?: number[];
  digitSize?: number;
  timeScale?: number;
...
export default function FaultyTerminalCards({
  scale = 1,
  gridMul = [2, 1],
  digitSize = 1.5,
  timeScale = 0.3,
